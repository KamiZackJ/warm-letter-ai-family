...
后端会把草稿版本号传给模型，并在八种写作方向之间轮换，包括具体瞬间切入、口语消息、时间顺序、短段落留白和生活细节起笔。第一轮负责个性化写作，第二轮以零随机度重新核对原始素材，删除推断出的身份、动作、效果和计划。因此每次生成会调用模型两次，成本和耗时会略有增加。最终输出仍会校验每段引用的用户素材，避免为了“个性”而编造事实。
