...
10. 公网 API 已部署提交 `8875dff`，启用 Qwen 双模型、Qwen 家书朗读和独立的生成/朗读费用限流。
...
13. `0.2.1` 关闭 Qwen 写作/复核默认额外思考，仍保留事实复核；同素材原配置 106.872 秒，对照 60.248 秒。完整隔离 API 首次生成 75.142 秒、核对转写后重生成 49.082 秒。各为单次测量，详见[真实复测报告](REAL_MULTIMODAL_TEST_2026-09-18.md)。
14. 新增作者私有语音转写核对、更正后重新生成、旧稿发布/持久朗读拦截，以及图片待核对标记。正常结束的空草稿最多补试一次，复用素材及转写。

当前阻塞：开发者工具提示“服务端口已关闭”，`0.2.1` 尚未上传或选为体验版。先开启工具“设置 → 安全设置 → 服务端口”，再上传。服务端修复已生效；仍需真机验证新编辑界面。本次重启会丢失旧内存会话和测试家书，需重新登录与生成。
...
1. 上传小程序 `0.2.1`，真机完成任意授权图片、文字和录音的上传、生成、语音纠错重生成、来源核对、预览、AI 朗读、确认、分享、阅读和回复闭环。
2. 新版本上传后需要再次“选为体验版”。体验好友必须先加入“体验成员”；`release` 仍保持生产地址未配置并失败关闭。
