...
2. 文件素材先请求短期上传地址，客户端通过真实 HTTP PUT 直传后调用完成接口；文字素材通过 `POST /v1/materials` 提交。当前本地演示使用 `FileSystemObjectStorage`，生产再替换为 S3/OSS 预签名上传。API 只把校验成功且属于当前用户的素材标为 `READY`。
...
- 孵化潜力证据：说明内存仓库、Fake AI 和本地文件系统对象存储分别可替换为 PostgreSQL、真实 AI 和 S3/OSS；展示分享令牌生命周期、异步媒体任务和成本/留存指标的扩展契约。
