...
- 公网 API 仍必须保持 `AI_PROVIDER=fake`；尚未把任何真实 AI 密钥写入服务器。
...
- 现有结果只达到健康检查标记的 `profile-match`：当前配置匹配经过合成探针的档案，但不证明当前凭据、网络或供应商运行时持续可用，也不等于授权四素材同封家书端到端、正式隐私审核、费用验收或公网切换完成。
...
同日还从编译后的真实 Provider 代码执行了一次完整合成闭环：`qwen3.5-omni-flash` 流式转写仓库合成 WAV，随后 `qwen3.8-flash` 联合读取合成图片、转写和文字，完成草稿生成与第二轮事实审校。单次耗时约 112 秒，三份素材 ID 均进入最终 `sourceRefs`。脱敏结果保存在本机 `D:\tmp\warm-letter-ai-family\qwen-provider-e2e-result.json`，不进入公开 Git；密钥未写入结果。
...
用于探针的密钥已经在协作会话中提供，必须先在供应商后台撤销或轮换。即使旧密钥仍可调用，也不得用于演示、服务器或生产。
...
以下是通过合成探针的精确配置，不是当前公网配置。必须使用轮换后的新密钥，并且只写入服务器私密环境文件：
...
可以说：“暖笺已经完成阿里云百炼北京地域 Qwen 双模型的合成多模态探针，并在代码中锁定已验证配置。公网目前仍使用确定性 Fake AI，待密钥轮换、隐私与费用审核及授权四素材端到端验收后再切换。”

不可以说：“公网已经使用真实 Qwen”“队友真实照片和语音已经通过 Qwen 处理”“已完成生产放行”，也不可以把历史第三方代理表述为官方 Gemini。
