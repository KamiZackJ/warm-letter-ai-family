...
三次对照的独立记录已清理，临时 HTTP 和 HTTPS 图片路由都已回滚，主 Caddy/API 未重启。准备一次可信免费 CA 的独立预签发对照，旧有效证书继续服务；尚未签发或启用新证书。测试期间发现 Caddy adapter 自动生成的配置文件隐藏路径和互斥组编号随输入文件/新增站点改变，已对比较值做精确规范化后重新验证；运行配置未发生未知业务改动，真实配置与摘要仍严格核对。
