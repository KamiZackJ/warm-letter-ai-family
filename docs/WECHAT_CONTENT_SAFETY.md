...
2026-09-23 23:40:26（北京时间）刷新同一测试账号的微信登录。23:40:57.390 单次提交阿里云文档公开 JPEG 示例（496,395 字节、无鉴权及查询参数）；23:41:00.965 收到经过认证的 `pass` 回调，约 3.575 秒。此结果证明当时该账号可提交图片审核、微信可下载该 OSS 图片并回传通过结果，不能证明当前 API 主机或真实素材已通过。

对照使用独立 `UPLOADING` 诊断记录，不含对象存储键、不关联家书；完成后仅清理本次诊断记录。真实照片与录音仍保持失败状态，未被对照结果放行。没有重新调用 AI。

生产 URL 只读检查：运行模块生成的地址为 173 个 ASCII 字符，只有 `expires` 和 64 位十六进制签名，2700 秒有效期；本机签名校验接受、JSON 往返不变。照片/音频类型分别为 2/1，MIME 为 JPEG/MP3。历史完整出站地址未留存，因此这不是历史请求原文证据。

23:46:21.746 向微信单次提交 API 主机上的公开合成 PNG 对照，23:46:25.495 收到 `failed/-1008`。该图片仅 8,287 字节，使用无查询参数、有 `.png` 后缀的精确静态路径；严格 HTTPS 自检为 200，TLS 配置和 Caddy 进程未改变。故不能仅把故障归因于素材签名查询、文件大小或缺少扩展名；当前主机到微信下载侧的访问原因仍需排查。这次独立诊断记录也已清理，没有放行真实素材。

2026-09-24 00:04:22.623（北京时间）将同主机、相同字节的公开合成 PNG 以临时 HTTP 精确路径单次送审，00:04:27.484 收到 `pass`，约 4.861 秒。请求没有重定向；其他 HTTP 路径仍跳转 HTTPS，账号、真实素材、微信令牌及审核 API 未改用 HTTP。两次同主机对照的随机路径标识不同，静态处理和文件相同；这是 HTTPS 访问链路方向的证据，不能单独证明某个 CA 或某个 TLS 算法有错。

三次对照的独立记录已清理，临时 HTTP 和 HTTPS 图片路由都已回滚，主 Caddy/API 未重启。测试期间发现 Caddy adapter 自动生成的配置文件隐藏路径和互斥组编号随输入文件/新增站点改变，已对比较值做精确规范化后重新验证；运行配置未发生未知业务改动，真实配置与摘要仍严格核对。

2026-09-24 00:31 独立预签发取得 ZeroSSL RSA DV SSL CA 2 的候选证书；域名、系统信任链、RSA 2048 密钥匹配验证通过。00:32 短时切换实际返回的新证书，SNI／无 SNI 和 TLS 1.2／1.3 均验证其指纹。00:32:52.941、00:32:53.305 对现有真实照片和录音重新提交检查，00:32:57.820、00:32:57.944 两项实际回调仍为 `failed/-1008`。此对照说明更换 CA 未解决下载问题，不能仅将现象归因于原 LE 链。随后恢复原 LE 配置与自动管理，无 API 重启、素材重传或新 AI 生成；失败记录未放行。下一步仅定位 HTTPS 服务实现兼容性。

补充的 TLS 实现对照：2026-09-24 00:49 暂由官方 Ubuntu Nginx/OpenSSL 在同一公网 443 接收 API 流量，保留原 LE 证书和 TLS 1.2/1.3、仅启用现代 AEAD 套件，再严格 HTTPS 回代理原 Caddy；Windows 新连接的 `/health` 响应头证实已进入该路径。真实照片和录音分别于 00:49:26.323、00:49:26.714 提交，00:49:29.865、00:49:30.798 仍回调 `failed/-1008`。该窗口访问日志仅见确认 POST 409 和两条回调 POST 200，没有素材 GET/HEAD；warn 级错误日志为空，不能据此排除仅在 info 级别记录的握手中止。临时 NAT 规则及 Nginx 已回滚，Caddy 配置、API 和数据库保持原状，公网健康正常。因此既不能认定 Caddy 是唯一原因，也不能直接断言微信端封禁该域名。
