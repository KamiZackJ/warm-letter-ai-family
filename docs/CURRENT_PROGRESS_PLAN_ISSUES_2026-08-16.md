...
小程序独立复核确认 401 单飞/单次重放、稳定素材键和 reader 同页回复重试主路径成立，但发现两个 P1：素材重读可在写操作后用旧 `selectedIds` 快照覆盖刚保存的新素材；家书失效 ID 清理可用旧列表覆盖并发新增的家书 ID。另有 Mock 素材/回复幂等契约不一致、待重试身份只活在页面实例、录音停止缺少进行中锁等 P2/真机边界。小程序完整批次在两个 P1 修复并补并发测试前不能提交。当前 Web/小程序代码仍处于混合工作树中，尚未拆分提交、推送或进入远端 CI。
...
| 小程序素材页 | 已实现最新加载获胜、旧数据保留、加载期禁用动作、卸载停止录音/阻止 UI 写入，以及不可变失败快照：“重试原内容”复用原 ID 和原文字，修改后显式加入会生成新 Material ID；素材页 `20/20` 通过 | P1：`retryLoadMaterials()` 未与 `busyAction` 互斥，慢速重读可在保存后用旧选择快照覆盖新素材/新 ID；需修复并补时序测试。另需验证录音停止双击和卸载丢响应 |
| 小程序服务与 401 恢复 | 已实现素材上传丢响应恢复与稳定幂等键；业务请求遇到 401 后单飞 `wx.login`，并发请求共享登录，原操作最多重放一次，第二次 401 清 token 并上抛；定向测试已覆盖 | P1：`listLetters()` 清理失效 ID 时直接写回旧快照，可能删除并发 `createLetter()` 新增的 ID；应基于写回时最新列表只删已确认失效项。401 仍缺正式微信运行态证据 |
...
1. 修复素材重读/写入竞态：写操作期间禁止或合并 `retryLoadMaterials()`，避免旧 `selectedIds` 快照覆盖新素材和选择存储；补“保存先完成、慢读取后返回”的测试。
2. 修复家书 ID 原子清理：`listLetters()` 写回前重新读取最新 ID，只删除已确认失效项；补“慢读取期间并发创建家书”的测试。
3. 收口 Mock 幂等契约：同素材 ID 异内容返回冲突，回复同键重放原回复/异内容冲突；补 Mock 契约测试。为录音停止增加 `stoppingRecord` 或等价互斥并验证双击/乱序回调。
4. 明确“同页重试”边界：决定是否持久化素材/回复待重试身份；若暂不实现，产品与验收文档必须明确卸载后不保证恢复同一幂等键。
5. 评估并处理小程序收信页 `.reply-input` 固定 `30rpx` 未随 `font-large/font-extra` 放大的适老一致性问题。
6. 使用 `D:\tmp\warm-letter-ai-family` 作为唯一任务临时目录，复跑 API、Web、小程序、contracts 全量测试与各端 typecheck，并把结果绑定到冻结的提交 SHA/tree。
7. 复跑根级 `pnpm check`、`pnpm build`、Web `build:test` 与 production bundle verifier，执行敏感信息扫描和 `git diff --check`。
8. 按“鉴权与列表原子清理”“素材幂等与页面恢复”“reader/reply 可靠性”“Web 字号/回复/草稿保护”拆分提交；显式暂存文件，禁止 `git add -A`。
...
- P1：素材页重读与写操作可并发；`apps/miniprogram/src/pages/materials/index.ts:214-227` 的慢速读取在保存后返回时会用旧 `selectedIds` 回写页面和本地存储，`retryLoadMaterials()` 与模板重试按钮也未和 `busyAction` 互斥，使新素材从 UI 消失并可能删除新 ID。现有测试未覆盖该时序。
- P1：`apps/miniprogram/src/services/api.ts:421-433` 的 `listLetters()` 失效 ID 清理基于旧数组直接写回；慢速首页读取期间若 `createLetter()` 新增 ID，清理会覆盖该 ID，且当前没有服务端列表可恢复入口。
...
- 当前推进顺序固定为：两项小程序 P1 竞态返修 -> Mock/录音/适老边界收口 -> 全量门禁 -> 拆分提交 -> 每批远端双 CI -> 管理文档同步 -> 外部验收。
